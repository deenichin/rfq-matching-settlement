...
fn a_lag_term_alone_can_violate_the_four_term_timelock_inequality() {
    ...
    let base = Config { withdrawal_delay: Ts(40_000), max_quote_ttl: Ts(30_000), ..Config::default() };
    ...
    assert_eq!(base.block_time, Ts::ZERO);
    assert_eq!(base.max_indexer_lag, Ts::ZERO);
    assert_eq!(base.max_settlement_inclusion_time, Ts::ZERO);
    assert_eq!(base.validate(), Ok(()), "the two-term form is satisfied");

    // CONFIRMATIONS × block_time: 12 × 1s = 12s. 30 + 12 > 40.
    let confirmation_lag = Config { confirmations: 12, block_time: Ts(1_000), ..base };
    assert_ne!(confirmation_lag.block_time, Ts::ZERO, "the lag term under test must be non-zero");
    ...
    let indexer_lag = Config { max_indexer_lag: Ts(15_000), ..base };
    assert_ne!(indexer_lag.max_indexer_lag, Ts::ZERO);
    assert_eq!(indexer_lag.validate(), Err(ConfigError::WithdrawalDelayTooShort));

    // Inclusion time alone. 30 + 11 > 40.
    let inclusion = Config { max_settlement_inclusion_time: Ts(11_000), ..base };
    assert_ne!(inclusion.max_settlement_inclusion_time, Ts::ZERO);
    ...
    let exactly_equal = Config {
        confirmations: 3,
        block_time: Ts(1_000),
        max_indexer_lag: Ts(4_000),
        max_settlement_inclusion_time: Ts(3_000),
        ..base
    };
    ...
    let just_enough = Config { withdrawal_delay: Ts(40_001), ..exactly_equal };
...
fn a_horizon_that_does_not_cover_request_ttl_plus_settling_time_fails_to_start() {
    ...
    let violating = Config {
        ...
        min_horizon: Ts(360_000), // exactly equal: the inequality is strict.
        ..Config::default()
    };
    ...
    let shorter_still = Config { min_horizon: Ts(1_000), ..violating };
    assert_eq!(shorter_still.validate(), Err(ConfigError::HorizonTooShort));

    let just_enough = Config { min_horizon: Ts(360_001), ..violating };
    assert_eq!(just_enough.validate(), Ok(()));
}
...
fn overflowing_terms_are_rejected_rather_than_wrapped() {
    ...
    let horizon_overflow =
        Config { max_request_ttl: Ts(u64::MAX), max_settling_time: Ts(1), ..Config::default() };
    assert_eq!(horizon_overflow.validate(), Err(ConfigError::HorizonTermOverflow));

    let timelock_overflow = Config {
        max_quote_ttl: Ts(u64::MAX),
        max_indexer_lag: Ts(1),
        min_horizon: Ts(u64::MAX),
        ..Config::default()
    };
